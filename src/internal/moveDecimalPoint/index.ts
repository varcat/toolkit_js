import { getDecimalsLength, splitIntegerDecimal, toNumberStr } from "../../functions";

export const moveDecimalPoint = (x: string, p: number): string => {
  // 判断输入的数值是否为字符串
  ...
  console.log(`x:${x}, p:${p}, flotLength:${flotLength}, num:${num}`);
  if (flotLength === 0) {
    num = `${int}${flot}`;
